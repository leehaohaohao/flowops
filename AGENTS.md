...
Profiles: `dev`, `prod`, `local`. Configuration resolves through a single documented chain, highest priority first: command-line args > JVM system properties (`-D`) > process environment (`docker run -e`, `--env-file`, exported shell vars) > `.env.<profile>` (falling back to `.env`) > `application-<profile>.yml` > `application.yml` and code defaults. Every key is looked up down that chain independently, so a key missing from one source falls through to the next. The only dotenv loader is `DotenvPostProcessor` (order `HIGHEST_PRECEDENCE + 5`, appended with `addLast`); it resolves the profile itself from `--spring.profiles.active`, `-Dspring.profiles.active`, `SPRING_PROFILES_ACTIVE`, then `prod`, because `environment.getActiveProfiles()` is still empty at that order in Spring Boot 3.3. Do not inject dotenv entries with `System.setProperty` from `main`: system properties outrank environment variables, which silently makes a mounted `.env.<profile>` override `docker run -e`. See `docs/configuration-loading-order.md` for the authoritative table, the container's effective sources, and the regression test. At startup the app logs a configuration-source table (key, defining source, placeholder filler, effective value) from `ConfigSourceReporter`; sensitive keys such as `spring.datasource.password` and `sa-token.jwt-secret-key` report only their source and never their value, and the report can be silenced with `flowops.config-report.enabled=false`. Keep `.env` values and runner tokens out of logs and commits. `application.yml` defaults to port 8080 and enables the Nexa master on `127.0.0.1:8081` (`NEXA_MASTER_HOST` / `NEXA_MASTER_PORT`); check profile overrides and host binding when using a remote runner. `deploy-prod.sh <jar> [profile] [env-file]` runs the app container: it publishes the console port (host 8880 by default, `PORT`) and the master-to-runner port (container and host 8081 by default, `NEXA_MASTER_PORT` for the container side and `NEXA_PORT` for the host side), and sets `NEXA_MASTER_HOST=0.0.0.0` through `-e` so runners on other hosts can connect (override via the shell environment).
